...
-- Solution
WITH avg_comparison_list AS (

    SELECT
        business_id,
        IF (occurences > (AVG(occurences) OVER (PARTITION BY event_type)), 1, 0) AS greater_than_avg
    FROM Events

)

SELECT business_id
FROM avg_comparison_list
WHERE greater_than_avg = 1
