...
-- Solution
WITH cum_weight_list AS (

    SELECT
        turn,
        IF (SUM(weight) OVER (ORDER BY turn ROWS UNBOUNDED PRECEDING) <= 1000, 1, 0) AS is_available
    FROM Queue
)

SELECT person_name
FROM Queue
WHERE turn = (SELECT MAX(turn) FROM cum_weight_list WHERE is_available = 1);
