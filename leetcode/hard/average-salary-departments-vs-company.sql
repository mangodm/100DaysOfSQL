...
SELECT 
    ...
    CASE 
        WHEN tmp.company_avg > department_salary.department_avg THEN 'lower'
        WHEN tmp.company_avg < department_salary.department_avg THEN 'higher'
        ELSE 'same' 
    END AS comparison
FROM tmp
LEFT JOIN (SELECT 
           ...
           INNER JOIN Salary
           USING (employee_id)
           GROUP BY Employee.department_id, LEFT(Salary.pay_date, 7)) AS department_salary
    ON tmp.pay_month = department_salary.pay_month;
