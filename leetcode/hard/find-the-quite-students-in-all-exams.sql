...
SELECT
    student_id,
    CASE
        WHEN score = MAX(score) OVER (PARTITION BY exam_id ORDER BY score DESC) THEN 1 
        WHEN score = MIN(score) OVER (PARTITION BY exam_id ORDER BY score) THEN 1 
        ELSE 0 
    END AS is_filtered
FROM Exam) tmp
LEFT JOIN Student
